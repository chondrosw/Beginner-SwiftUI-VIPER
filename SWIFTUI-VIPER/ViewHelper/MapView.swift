...
struct Map_Previews:PreviewProvider{
    static var previews: some View{
        let pins = DataModel.sample.trips[0].waypoints.map{
            waypoint -> MKPointAnnotation in
            let annotaion = MKPointAnnotation()
            annotaion.coordinate = waypoint.location
            return annotaion
        }
        return Group{
            MapView(pins: pins, routes: nil, center: nil).previewDisplayName("Pins")
//            MapView(pins: [], routes: nil, center: CLLocationCoordinate2D.timesSquare).previewDisplayName("Center")
        }
    }
}
#endif
