...
class Persistence{
    ...
    func save(trips:[Trip]){
        let enevelope = Envelope(trips: trips)
        let encoder = JSONEncoder()
        encoder.outputFormatting = .prettyPrinted
        let data = try! encoder.encode(enevelope)
    ...
    private func loadSynchronously(_ file:URL)-> [Trip]{
        do{
            ...
            let envelopee = try JSONDecoder().decode(Envelope.self, from: data)
            return envelopee.trips
        }catch{
            clear()
